...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSearch, isSearchOpen, totalDocs = 23 }) => {
  return (
    <header className="sticky top-0 z-30 bg-space-950/80 backdrop-blur-xl border-b border-white/10 px-6 py-3 flex items-center justify-between shadow-2xl">
      {/* Brand & Identity on Left (Clean, no search button) */}
      ...
      <div className="flex items-center space-x-2.5 text-xs">
        ...
        {/* Search & Intelligence Trigger on RIGHT */}
        <button
          onClick={onToggleSearch}
          className={`flex items-center space-x-2 px-3.5 py-1.5 rounded-xl border text-xs font-semibold transition-all duration-200 group cursor-pointer ${
            isSearchOpen
          ...
          title="Toggle Search & Intelligence Copilot (⌘K)"
        >
